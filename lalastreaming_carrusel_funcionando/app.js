...
if (typeof buyNowFloat !== "undefined")
  buyNowFloat.href = `https://wa.me/51${store.whatsapp}?text=${encodeURIComponent("Hola, quiero comprar una plataforma")}`;
if (typeof featuredWa !== "undefined")
  featuredWa.href = `https://wa.me/51${store.whatsapp}?text=${encodeURIComponent("Hola, quiero consultar la oferta destacada de Netflix")}`;
terms.innerHTML = store.terms
  .split("\n")
  .map((t) => `<p>${t}</p>`)
  .join("");
yapeNumber.textContent = store.yape || store.whatsapp;
plinNumber.textContent = store.plin || store.whatsapp;
if (typeof clientsCounter !== "undefined")
  clientsCounter.textContent = "+" + (store.clientsCount || 250);
  ...
  if (carousel) {
    ...
    carousel.innerHTML += `
      <div class="carousel-item client-slide">
        ...
        <span>Soporte rápido y atención personalizada.</span>
      </div>
    `;
...
function priceText(p) {
  ...
  return currency === "USD"
    ? "$ " + (n / (Number(store.exchangeRate) || 3.75)).toFixed(2)
    : "S/ " + n.toFixed(2);
}
function waLink(product) {
  const msg = (
    store.whatsappMessage || "Hola, quiero comprar {producto}."
  ).replaceAll("{producto}", product.name);
  return `https://wa.me/51${store.whatsapp}?text=${encodeURIComponent(msg)}`;
}
function logoBlock(p) {
  return `<div class="brand-logo" style="--brand:${p.color || "#9b5cff"}">${p.logo || p.name}</div>`;
}
...
function renderFilters() {
  ...
  data.categories.forEach(
    (c) =>
      (categoryFilters.innerHTML += `<button class="${selectedCategory === c.id ? "active" : ""}" data-cat="${c.id}">${c.name}</button>`),
  );
...
function render() {
  ...
  const uncategorized = filtered.filter(
    (p) => !data.categories.some((c) => c.id === p.category),
  );
  if (uncategorized.length)
    catalogByCategory.innerHTML += `<div class="category-section"><h3>Sin categoría</h3><div class="grid">${uncategorized.map(productCard).join("")}</div></div>`;
...
currencyBtn.onclick = () => {
  ...
  currencyBtn.innerHTML =
    currency === "PEN" ? "💵 Ver precios en USD" : "🇵🇪 Ver precios en Soles";
  render();
};
search.oninput = () => {
